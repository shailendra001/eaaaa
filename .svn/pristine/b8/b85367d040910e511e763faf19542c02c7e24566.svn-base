...
#import "EDStarRatings.h"

#define ED_DEFAULT_HALFSTAR_THRESHOLD   0.6


@interface EDStarRatings()
...
-(void)setDefaultProperties
{
        ...
        displayMode = EDStarRatingDisplayFull1;
...
-(void)setDelegate:(id<EDStarRatingProtocols>)delegate
{
        _delegate = delegate;
        _returnBlock = nil;
}
                ...
                if( i < _rating )   // Highlight
                {
                        CGContextSaveGState(ctx);
                        {
                                if( i< _rating &&  _rating < i+1 )
                                {
                                        ...
                                        if( displayMode == EDStarRatingDisplayHalf1 && difference < halfStarThreshold )    // Draw half star image
                                        {
                                                rectClip.size.width/=2.0;
                                        }
                                        else if( displayMode == EDStarRatingDisplayAccurate1 )
                                        {
...
-(float) starsForPoint:(CGPoint)point
{
        ...
        for( NSInteger i=0; i<maxRating; i++ )
        {
                ...
                if( point.x > p.x )
                {
                        ...
                        if( self.displayMode == EDStarRatingDisplayHalf1  )
                        {
...
#endif
{
        ...
        if( self.delegate && [self.delegate respondsToSelector:@selector(starSelectionChanged:rating:)] )
                [self.delegate starSelectionChanged:self rating:self.rating];
