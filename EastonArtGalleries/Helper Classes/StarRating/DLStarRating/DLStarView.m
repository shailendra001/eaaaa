...
- (id)initWithDefault:(UIImage*)star highlighted:(UIImage*)highlightedStar position:(int)index allowFractions:(BOOL)fractions {
	...
	if (self) {
        ...
        if(kNumberOfFractions>2){
                self.frame = CGRectMake((star.size.width*index),
                                        0,
                                        star.size.width-2.6,
                                        star.size.height-16+kEdgeInsetBottom);
        }
        else{
                self.frame = CGRectMake((star.size.width*index),
                                        0,
        ...
        [self setImageEdgeInsets:UIEdgeInsetsMake(16, 0, 16, 0)];
